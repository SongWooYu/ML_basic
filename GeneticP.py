...
def crossover(pop):
    father = select(pop)
    mother = select(pop)
    index = 2 #random.randint(1, SIZE - 1)
...
while population[0].cal_fitness() < 31:
    ...
    # 상위 20 부모 + 상위 20 자식을 남겨 다음 세대를 만든다.
    top_parents = sorted(population, key=lambda x: x.cal_fitness(), reverse=True)[:20]
    top_children = sorted(new_pop, key=lambda x: x.cal_fitness(), reverse=True)[:20]
    population = top_parents + top_children
