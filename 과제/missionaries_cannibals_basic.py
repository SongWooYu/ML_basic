...
# Command 정의: (선교사 수, 식인종 수) 최대 2명까지 이동 가능
COMMANDS = {
    "1M": (1, 0),
    "2M": (2, 0),
    "1C": (0, 1),
...
def judge(status):
    ...
    if status["right_m"] == 3 and status["right_c"] == 3:
        return True, "승리: 모두 오른쪽으로 이동했습니다."
...
# 최초 상태: 모두 왼쪽에 있고 배도 왼쪽에 있음
status = make_status(3, 3, 0, 0, "left")

while True:
    print_status(status)
    cmd = input("Command 입력: ").strip().upper()
