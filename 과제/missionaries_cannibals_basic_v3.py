...
# 시작 방향: 오른쪽 -> 왼쪽
# 개선: 잘못된 입력/불가능한 이동은 종료하지 않고 다시 입력받음
...
def game(status, command):
    ...
    if command not in COMMANDS:
        message = "입력 오류: 사용할 수 없는 Command입니다. 다시 입력하세요."
    ...
    if status["boat"] == "right":
        ...
        if status["right_m"] < move_m or status["right_c"] < move_c:
            message = "이동 불가: 오른쪽에 해당 사람 수가 부족합니다. 다시 입력하세요."
            return status, False, message
        ...
        if status["left_m"] < move_m or status["left_c"] < move_c:
            message = "이동 불가: 왼쪽에 해당 사람 수가 부족합니다. 다시 입력하세요."
            return status, False, message
