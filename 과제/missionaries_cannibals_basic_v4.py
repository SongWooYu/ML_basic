...
def judge(status):
    ...
    if not is_safe(status["left_m"], status["left_c"]):
        return True, "게임오버: 왼쪽 선교사가 식인종에게 먹혔습니다."

    if not is_safe(status["right_m"], status["right_c"]):
        return True, "게임오버: 오른쪽 선교사가 식인종에게 먹혔습니다."

    # 성공 조건: 모두 왼쪽으로 이동
    if status["left_m"] == TOTAL_M and status["left_c"] == TOTAL_C:
        return True, "성공했습니다: 모두 왼쪽으로 이동했습니다."

    return False, "계속 진행합니다."
...
def game(status, command):
    ...
    if command not in COMMANDS:
        return status, False, "입력 오류: 사용할 수 없는 Command입니다. 다시 입력하세요."
    ...
    if status["boat"] == "right":
        ...
        if status["right_m"] < move_m or status["right_c"] < move_c:
            return status, False, "이동 불가: 오른쪽에 해당 사람 수가 부족합니다. 다시 입력하세요."
        ...
        if status["left_m"] < move_m or status["left_c"] < move_c:
            return status, False, "이동 불가: 왼쪽에 해당 사람 수가 부족합니다. 다시 입력하세요."
...
try:
    while True:
        ...
        cmd_normalized = normalize_command(cmd)

        if cmd_normalized in ["Q", "QUIT", "EXIT"]:
            print("사용자가 게임을 종료했습니다.")
            break

        status, end, message = game(status, cmd)
        print(f"결과: {message}")
